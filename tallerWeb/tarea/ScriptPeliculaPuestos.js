...
function mostrarNotificacionError(mensaje) {
    // Crea una notificación con el mensaje de error
...
/*function mostrarNotificacionError(mensaje) {
    alert(mensaje); // Puedes reemplazar esto con tu propio método de notificación
}*/
...
async function reservarAsiento(seat) {
    ...
    try {
        ...
        if (respuesta.status === 200 || respuesta.status === 201) {
            ...
            // Aquí puedes agregar lógica adicional para manejar la reserva exitosa
...
/*async function recibirActualizacionesReservas(seatId) {
    const url = `${urlApi}/theatres/${theatreId}/auditoriums/${auditoriumId}/seats/${seatId}/reserve-update`;
    try {
        const respuesta = await fetch(url);
        if (respuesta.ok) {
            const actualizaciones = await respuesta.json();
            console.log('Actualizaciones recibidas:', actualizaciones);
            // Aquí puedes agregar lógica adicional para manejar las actualizaciones recibidas
        } else {
            throw new Error(`Error HTTP: ${respuesta.status}`);
        }
    } catch (error) {
        console.error('Hubo un problema al recibir las actualizaciones de reservas:', error);
        mostrarNotificacionError(error.message);
    }
}*/

/*async function recibirActualizacionesReservas(theatreId, auditoriumId, showtimeId) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log('Actualizaciones recibidas:', jsonResponse);
        } else {
    ...
    botonesAsiento.forEach((boton) => {
        boton.addEventListener('click', async (event) => {
            ...
            if (boton.classList.contains('seleccionado')) {
                ...
                // Llama a la función para reservar el asiento
                try {
